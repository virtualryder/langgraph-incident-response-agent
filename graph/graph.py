...
def build_graph(
    ...
    # Linear: triage → rag → tool → generate_hypotheses
    workflow.add_edge("triage_agent", "rag_agent")
    workflow.add_edge("rag_agent", "tool_agent")
    ...
    # Linear: synthesize → blast_radius → compliance → CAB
    ...
    print("[Graph] Nodes: triage → rag → tools → hypotheses(parallel) → blast_radius → compliance → CAB(interrupt) → action → postmortem")
